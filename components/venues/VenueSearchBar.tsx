...
export function VenueSearchBar({ defaults = {}, compact = false }: VenueSearchBarProps) {
  ...
  return (
    <form
      onSubmit={submit}
      className={`venue-search-grid grid w-full grid-cols-[minmax(0,1fr)] overflow-hidden rounded-[22px] border border-foreground/15 bg-card shadow-[0_18px_55px_rgba(20,19,13,0.12)] ${compact ? "lg:grid-cols-[1.2fr_1fr_.75fr_.8fr_auto]" : "lg:grid-cols-[1.35fr_1fr_.8fr_.8fr_auto]"}`}
      aria-label="Search venues"
    >
      <label className="group flex min-w-0 flex-col px-4 py-3 hover:bg-secondary/50 lg:border-r lg:border-foreground/10">
      ...
      <label className="flex min-w-0 flex-col border-t border-foreground/10 px-4 py-3 hover:bg-secondary/50 lg:border-r lg:border-t-0">
      ...
      <select id="venue-event-type" value={eventType} onChange={(event) => setEventType(event.target.value)} className="min-w-0 max-w-full border-t border-foreground/10 bg-card px-4 py-3 text-sm font-semibold outline-none lg:hidden">
