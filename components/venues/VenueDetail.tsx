...
export function VenueDetail({ initial }: VenueDetailProps) {
  const reduceMotion = useReducedMotion();
  const initialSpace = initial.people <= 3
    ? TIME_CAFE.spaces.find((space) => space.id === "small-table")!
    : initial.people <= 4
      ? TIME_CAFE.spaces.find((space) => space.id === "standard-table")!
      : TIME_CAFE.spaces[0];
  const [spaceId, setSpaceId] = useState<VenueSpaceId>(initialSpace.id);
  ...
  const space = useMemo(
    () => TIME_CAFE.spaces.find((item) => item.id === spaceId) ?? TIME_CAFE.spaces[0],
    [spaceId],
  );
    ...
    <>
      <div className="mx-auto max-w-7xl px-4 pb-24 pt-6 sm:px-6 lg:px-8 lg:pb-0 lg:pt-9">
        <nav className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground" aria-label="Breadcrumb">
          ...
          <span>/</span>
          <span className="text-foreground">Time Cafe</span>
        </nav>
        ...
        >
          <div>
            <VenueKicker>Nungambakkam · Cafe &amp; gathering space</VenueKicker>
            <h1 className="mt-2 font-display text-5xl font-black tracking-[-0.065em] sm:text-6xl lg:text-7xl">Time Cafe</h1>
            <div className="mt-3 flex flex-wrap items-center gap-3 text-sm">
              <a href={TIME_CAFE.caféRatingUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 font-bold underline decoration-foreground/20 underline-offset-4">
                <VenueIcon name="star" className="size-4 fill-current" /> {TIME_CAFE.caféRating} dining rating
              </a>
              <span className="text-foreground/20">•</span>
              <span className="inline-flex items-center gap-1.5 font-semibold"><VenueIcon name="people" className="size-4" /> Up to 30 guests</span>
              <span className="text-foreground/20">•</span>
        ...
        >
          <div className="relative h-52 overflow-hidden bg-[#e7e6dc] sm:h-56" role="img" aria-label="Map preview showing Time Cafe near Valluvar Kottam High Road in Nungambakkam">
            ...
            <span className="absolute left-[11%] top-[67%] rounded-full bg-card/90 px-2.5 py-1 text-[10px] font-semibold text-muted-foreground shadow-sm">Valluvar Kottam High Road</span>
            <span className="absolute right-[8%] top-[15%] rounded-full bg-card/85 px-2.5 py-1 text-[10px] font-semibold text-muted-foreground">Nungambakkam</span>
            <motion.div initial={reduceMotion ? false : { y: -12, scale: 0.8, opacity: 0 }} animate={{ y: 0, scale: 1, opacity: 1 }} transition={{ delay: 0.38, type: "spring", stiffness: 380, damping: 22 }} className="absolute left-[53%] top-[46%] -translate-x-1/2 -translate-y-1/2">
              <div className="relative grid size-12 place-items-center rounded-full border-4 border-card bg-primary text-white shadow-[0_10px_25px_rgba(196,27,9,.28)]"><VenueIcon name="coffee" className="size-5" /><span className="absolute -bottom-7 whitespace-nowrap rounded-full bg-foreground px-2.5 py-1 text-[10px] font-bold text-background">Time Cafe</span></div>
            </motion.div>
            <a href={TIME_CAFE.mapUrl} target="_blank" rel="noreferrer" className="absolute left-3 top-3 rounded-full border border-foreground/15 bg-card/95 px-3 py-2 text-xs font-bold shadow-sm backdrop-blur hover:border-foreground">Open in Maps ↗</a>
          </div>
          <div className="flex flex-col justify-between border-t border-foreground/12 p-5 lg:border-l lg:border-t-0 lg:p-6">
            <div>
              <VenueKicker>Central Chennai</VenueKicker>
              <h2 className="mt-2 font-display text-2xl font-black tracking-[-0.04em]">Easy to reach. Easy to find.</h2>
              <p className="mt-3 text-sm leading-6 text-muted-foreground">{TIME_CAFE.address}</p>
            </div>
            <a href={TIME_CAFE.mapUrl} target="_blank" rel="noreferrer" className={`${venueButton.outline} mt-5 w-fit`}>
          ...
          <div className="min-w-0">
            ...
            <motion.section {...reveal} className="border-t border-foreground/15 py-11">
              ...
              <div className="mt-6 grid gap-px overflow-hidden rounded-[22px] border border-foreground/15 bg-foreground/15 sm:grid-cols-2">
                {TIME_CAFE.amenities.map((amenity, index) => (
            ...
            <motion.section {...reveal} className="border-t border-foreground/15 py-11">
              <VenueSectionHeading
                ...
                description="Only organizers who completed a booking through SCENE can leave one."
              />
              <VenueReviews venueName={TIME_CAFE.name} />
            </motion.section>
            ...
            <motion.section {...reveal} className="border-t border-foreground/15 py-11">
              ...
              <div className="mt-5 divide-y divide-foreground/12 rounded-[22px] border border-foreground/15 bg-card px-5">
                {TIME_CAFE.policies.map((policy, index) => (
